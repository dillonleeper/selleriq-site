...
export function ProductStage() {
  ...
  return <div className="product-window relative overflow-hidden rounded-[1.6rem] border border-white/10 bg-[#0d1a2d]/95 shadow-[0_40px_100px_rgba(0,0,0,.55)] backdrop-blur-xl">
    <div className="flex h-12 items-center justify-between border-b border-white/10 px-4"><div className="flex gap-1.5" aria-hidden>{["bg-red-400","bg-amber-400","bg-emerald-400"].map(c=><span key={c} className={`h-2 w-2 rounded-full ${c}`}/>)}</div><span className="font-mono text-[10px] uppercase tracking-[.2em] text-slate-500">SellerIQ / Overview</span><span className="h-2 w-2 rounded-full bg-emerald-400 shadow-[0_0_12px_#34d399]" /></div>
    <div className="grid min-h-[570px] md:grid-cols-[8.5rem_1fr]">
      <aside className="hidden border-r border-white/10 p-3 md:block"><div className="mb-6 px-2 pt-2 text-xs font-semibold">SELLERIQ</div>{["Overview","Sales","Inventory","Advertising","Profitability"].map((item,i)=><div key={item} className={`mb-1 rounded-lg px-2 py-2 text-[11px] ${i===0?"bg-blue-500/15 text-blue-300":"text-slate-500"}`}>{item}</div>)}<div className="mt-8 border-t border-white/10 pt-5"><p className="px-2 text-[9px] uppercase tracking-wider text-slate-600">Marketplaces</p><p className="mt-3 px-2 text-[11px] text-slate-400">🇺🇸 United States</p><p className="mt-2 px-2 text-[11px] text-slate-600">🇨🇦 Canada</p></div></aside>
      <div className="p-4 sm:p-6">
        ...
        <div className="mt-5 grid gap-4 xl:grid-cols-[1fr_.72fr]">
          ...
          <div className="flex flex-col rounded-2xl border border-amber-300/20 bg-amber-300/[.06] p-5"><div className="flex items-center justify-between"><span className="rounded-md bg-amber-300/15 px-2 py-1 text-[9px] font-semibold uppercase tracking-wider text-amber-200">Needs review</span><Sparkles className="h-4 w-4 text-amber-200"/></div><p className="mt-5 text-base font-semibold">Review a potential stock risk</p><p className="mt-2 text-xs leading-5 text-slate-400">Organic Tea has 14 days of FBA cover while recent sales velocity is rising.</p><div className="my-5 space-y-3 border-y border-white/10 py-4 text-[11px]"><p className="flex justify-between"><span className="text-slate-500">Sales velocity</span><span className="text-emerald-300">+22.8%</span></p><p className="flex justify-between"><span className="text-slate-500">Stock cover</span><span className="text-amber-200">14 days</span></p></div><button onClick={()=>setReviewed(true)} disabled={reviewed} className={`mt-auto rounded-xl px-3 py-2.5 text-xs font-semibold ${reviewed?"bg-emerald-400/15 text-emerald-300":"bg-blue-500 text-white hover:bg-blue-400"}`}>{reviewed?"Reviewed ✓":"Review recommendation"}</button></div>
        </div>
